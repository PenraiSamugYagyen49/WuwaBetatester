...
export default function DepthText({
  text = 'Elevate', layers = 34, depth = 2.4, faceColor = '#f8fafc', depthColor = '#7c3aed',
  tilt = 7.5, pointerTracking = true, smoothing = 0.14, perspective = 900,
  autoOrbit = true, orbitSpeed = 0.35, fontSize = 'clamp(3rem, 12vw, 7rem)',
  ...
  useEffect(() => {
    ...
    const tick = now => {
      if ((!canTrackPointer || !activePointer) && autoOrbit) {
        ...
        const fallbackAmount = canTrackPointer ? .18 : .55;
        target.x = baseRotation.x + Math.sin(orbit) * safeTilt * fallbackAmount;
        target.y = baseRotation.y + Math.cos(orbit * .85) * safeTilt * fallbackAmount;
      }
  ...
  const rootStyle = {
    ...
    '--depth-text-shadow': shadow
      ? `0 22px 34px color-mix(in srgb, ${depthColor} 36%, transparent), 0 4px 8px rgba(0,0,0,.28)`
      : 'none'
  };

  return <span ref={rootRef} className={`depth-text ${className}`.trim()} style={rootStyle}>
    <span ref={stageRef} className="depth-text__stage" aria-label={text}>
      ...
      <span className="depth-text__face" aria-hidden="true">{text}</span>
    </span>
  </span>;
}
