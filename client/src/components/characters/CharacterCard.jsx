...
export default function CharacterCard({ character, index, onNavigate }) {
  ...
  return <article className="character-card" style={{ '--element-color': elementColors[character.element] }} tabIndex="0" role="link" onClick={open} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open(); } }}>
    {character.image && <img src={character.image} alt={`${character.name} artwork`} loading={index < 10 ? 'eager' : 'lazy'} fetchPriority={index < 5 ? 'high' : 'auto'} decoding="async" />}
    ...
    <div className="character-card__content"><small>{String(index + 1).padStart(2, '0')} · Resonator</small><h2>{character.name}</h2><p aria-label={`${character.rarity} star rarity`}>{'★'.repeat(character.rarity)}</p></div>
  </article>;
}
